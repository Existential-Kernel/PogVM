...
inline void DECODE::Push(std::vector<uint8_t> &v, const u_char &s, uint8_t &arg, const uint8_t &count) {
    v.push_back(s);
    arg = count;
}

inline void DECODE::SinglePush(std::vector<u_char> &v, const u_char &s, std::vector<std::vector<u_char>> &instructions) {
    v.push_back(s);
    instructions.push_back(v);
}

void DECODE::CheckBits(const uint8_t &acceptablebits, const uint8_t &kernelbits, const uint8_t &hex) {
...
void DECODE::CheckProc(const uint8_t &acceptableproc, const uint8_t &kernelproc) {
...
void DECODE::Decode(const std::vector<uint8_t> &hexvector, std::vector<std::vector<uint8_t>> &resultvector, const uint8_t &bits, const uint8_t &processor) {
